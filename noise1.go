...
var PROXIES []string
var CUSTOM_HEADERS map[string]string
var cur int32
        ...
        if headerFile != "nil" {
                ...
                if err == nil {
                        ...
                        for _, l := range lines {
                                ...
                                if len(parts) == 2 {
                                        CUSTOM_HEADERS[strings.TrimSpace(parts[0])] = strings.TrimSpace(parts[1])
                                }
                        }
                }
        }

        fmt.Println(" *--------* Noise Tool Started *---------*")
        ...
        for i := 0; i < threads; i++ {
                ...
                go func() {
                        ...
                        for atomic.LoadInt32(&stopFlag) == 0 {
                                if atomic.LoadInt32(&cur) >= int32(threads) {
...
func sendRequest(target, method string, s chan uint8, stopTime time.Time, stopFlag *int32) {
        ...
        if atomic.LoadInt32(stopFlag) == 1 || time.Now().After(stopTime) {
                return
        }
        ...
        postData := ""
        for _, k := range keys {
                postData += fmt.Sprintf("%s=%s&", k, buildblock(3+rand.Intn(5)))
        }
...
func detectWAF(resp *http.Response, body string) string {
        ...
        for _, h := range WAF_HINTS {
                if strings.Contains(bl, h) {
                        return h
                }
        }
        switch resp.StatusCode {
        case 403:
                return "Generic 403 Block"
        case 406:
                return "Request Blocked (406)"
        case 503:
